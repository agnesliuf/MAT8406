...
subsets<-regsubsets(mortality~precip+jantemp+    
                      house+educ+density+nonwhite+poor+     
                      lognox+logso2+I(jantemp^2), data=dat2)

plot(subsets)               # Examine BIC values

plot(subsets, scale="Cp")   # Examine Cp values
# Inspect Cp Plot
# ================
mallow<-leaps(dat[,-c(1,2),2],dat$mortality)         
plot(mallow$size, mallow$Cp,      
     main="Cp Plot",xlab="p", ylab="Cp", 
     xlim=c(2,10), ylim=c(0,15),
     pch=19, cex=1.4) 
abline(0, 1,lwd=2)                 
...
# Part (e)
# ========
round(hatvalues(reg6),4)         # Levarage values
round(rstudent(reg6),4)          # Studentized deleted residuals  
